...
if (isset($tag) && $tag){
    ...
    if ($tag){
        ...
        if ($global_settings['seo_links']){
            $pagination = $misc->getBasicPagination($category_count,$p,$maxperpage,$baseurl."/".$routes['movie_tag']."/".$tag_perma."/".$sortby."/");
        } else {
            $pagination = $misc->getBasicPagination($category_count,$p,$maxperpage,$baseurl."/index.php?menu=movie-tag&tag=$tag_perma&sortby=$sortby&p=");
        }

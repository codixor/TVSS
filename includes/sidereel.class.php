...
class Sidereel{
  ...
  public function getThumbnail($show,$season,$episode,$basepath){
    ...
    /*    if (substr_count($page,"<?xml version")){
      ...
      if ($filename && $id){
        $imageurl = "http://s4.sidereel.com/episodes/$id/featured/$filename";
      }
      ...
      for($i=0;$i<$divs->length;$i++){
        if ($divs->item($i)->getAttribute("class")=='episode-image four elevenths'){
    ...
    if ($page){
      if (substr_count($page,"DOCTYPE html")){
        ...
        for($i=0;$i<$divs->length;$i++){
         ...
         if (substr_count($divs->item($i)->getAttribute("class"),"episode")){
          $hs2 = $divs->item($i)->getElementsByTagName("h2");
          if ($hs2->length){
            $title = trim($hs2->item(0)->textContent);
          }
            
          $title= trim(str_replace($title,"",$divs->item($i)->textContent));

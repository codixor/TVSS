...
class Misc{
	...
	public function deleteLink($link_id){
		$del = ORM::for_table('submitted_links')->find_one($link_id)->delete();
	}
	
	public function approveLink($link_id){
		$upd = ORM::for_table('submitted_links')->find_one($link_id);
		$upd->set('status', 1);
		$upd->save();
	}
	...
	public function saveLink($params){
		...
		$embed_language = $params['embed_language'];
		...
		if (count($imdb_id)==2){
			...
			$link = strip_tags($params['video_url']);
			...
			$type = $params['type'];
			$check = ORM::for_table('submitted_links')->where('link', $link)->find_one();
			if (!$check){
				$ins = ORM::for_table('submitted_links')->create();
				$ins->user_id 		 = $user_id;
				$ins->type 			 = $type;
				$ins->imdb_id 		 = $imdb_id;
				$ins->season 		 = $season;
				$ins->episode 		 = $episode;
				$ins->link 			 = $link;
				$ins->language		 = $embed_language;
				$ins->date_submitted = $date_submitted;
				$ins->status 		 = 0;
				$ins->save();
			}
		}

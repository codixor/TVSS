...
class Parser {
        ...
        if ($codeSplit)
            ...
            foreach ($codeSplit as $html) {
                ...
                elseif (preg_match($tagMatch['loop'], $html, $matches)) {
                    ...
                    //loop code
                    $parsedCode .= "<?php $counter=-1; $assignNewVar if( !empty($newvar) && ( is_array($newvar) || $newvar instanceof Traversable ) && sizeof($newvar) ) foreach( $newvar as $key => $value ){ $counter++; ?>";
                }
    ...
    protected function modifierReplace($html) {
        ...
        if (strpos($html,'|') !== false && substr($html,strpos($html,'|')+1,1) != "|") {
            ...
            $function = $explode[0];
            $params = !empty($explode[1]) ? "," . $explode[1] : null;

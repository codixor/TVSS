...
if ($maxperpage){
	...
	if ($global_settings['seo_links']){
		$pagination = $misc->getBasicPagination($count,$p,$maxperpage,$baseurl."/".$routes['movies']."/$sortby/");
	} else {
		$pagination = $misc->getBasicPagination($count,$p,$maxperpage,$baseurl."/index.php?menu=movies&sortby=$sortby&p=");
	}
} else {

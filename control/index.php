...
if (isset($dologin)){ 
	$new_adminpass = $adminpass; 
	$old_adminpass = md5($adminpass); 
	
	$e = ORM::for_table('admin')->select_many('id', 'password', 'username')->where('username', $adminuser)->find_one();
	
	if ($e){		
		$stored_password = $e->password; 
		
		if(password_needs_rehash($stored_password, PASSWORD_BCRYPT) && $old_adminpass === $stored_password){ 
		
			$stored_password = password_hash($new_adminpass, PASSWORD_BCRYPT);			
			$e->set(['password' => $stored_password])->save();
			...
			if ($captchas && (empty($_SESSION['captcha']) || !isset($_REQUEST['captcha']) || trim(strtolower($_REQUEST['captcha'])) != $_SESSION['captcha'])){ 
				...
				$_SESSION['admin_user_id']  = $e->id; 
				$_SESSION['admin_username'] = $e->username; 
			} 
			
		} 
		elseif(password_verify($new_adminpass, $stored_password)){ 
			$captchas = $settings->getSetting("captchas"); 
